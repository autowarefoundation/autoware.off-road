...
def main():
    ...
    if load_from_checkpoint == False:
        trainer = FreespaceSegTrainer(pretrained_checkpoint_path=pretrained_checkpoint_path,
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        for count in tqdm(range(0, total_train_samples), desc=f"Training Epoch {epoch + 1} / {start_epoch + num_epochs}"):
            ...
            if ((count+1) % 8000 == 0) or ((count+1) == total_train_samples):
                ...
                running_IoU_full = 0
                running_IoU_non_drivable = 0
                running_IoU_drivable = 0
                ...
                with torch.no_grad():
                    ...
                    for dataset_name in DATASETS:
                        for val_count in tqdm(range(0, counts[dataset_name][1]),
                            ...
                            IoU_score_full, IoU_score_non_drivable, IoU_score_drivable = \
                                trainer.validate(image_val, gt_val)

                            running_IoU_full += IoU_score_full
                            running_IoU_non_drivable += IoU_score_non_drivable
                            running_IoU_drivable += IoU_score_drivable

                    # Calculating average loss of complete validation set
                    mIoU_full = running_IoU_full/total_val_samples
                    mIoU_non_drivable = running_IoU_non_drivable/total_val_samples
                    mIoU_drivable = running_IoU_drivable/total_val_samples

                    # Logging average validation loss to TensorBoard
                    trainer.log_IoU(mIoU_full, mIoU_non_drivable,
                                    mIoU_drivable, log_count)
