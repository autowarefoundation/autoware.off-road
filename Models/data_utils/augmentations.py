...
class Augmentations():
    def __init__(self, is_train: bool, data_type: DATA_TYPES_LITERAL):
        ...
        self.transform_noise = A.Compose(
            [
                A.MultiplicativeNoise(multiplier=(
                    0.5, 1.5), per_channel=False, p=0.5),
                ...
                A.GaussNoise(var_limit=(50.0, 100.0), mean=0,
                             noise_scale_factor=0.2, p=0.5),
                A.GaussNoise(var_limit=(250.0, 250.0), mean=0,
                             noise_scale_factor=1, p=0.5),
                A.ISONoise(color_shift=(0.1, 0.5),
                           intensity=(0.5, 0.5), p=0.5),
                A.RandomFog(fog_coef_lower=0.1, fog_coef_upper=0.3,
                            alpha_coef=0.2, p=0.25),
                A.RandomFog(fog_coef_lower=0.7, fog_coef_upper=1.0,
                            alpha_coef=0.04, p=0.25),
                A.RandomRain(p=0.1),
                A.Spatter(mean=(0.65, 0.65), std=(0.3, 0.3), gauss_sigma=(2, 2),
                          cutout_threshold=(0.68, 0.68), intensity=(0.3, 0.3), mode='rain',
                          p=0.1),
                A.ToGray(num_output_channels=3,
                         method='weighted_average', p=0.1)
            ]
        )
    ...
    def applyTransformSeg(self, image, ground_truth):
        ...
        if (self.data_type != 'SEGMENTATION'):
            raise ValueError(
                'Please set dataset type to SEGMENTATION in intialization of class')
    ...
    def applyTransformBinarySeg(self, image, ground_truth):
        ...
        if (self.data_type != 'BINARY_SEGMENTATION'):
            raise ValueError(
                'Please set dataset type to BINARY_SEGMENTATION in intialization of class')
    ...
    def applyTransformDepth(self, image, ground_truth):
        ...
        if (self.data_type != 'DEPTH'):
            raise ValueError(
                'Please set dataset type to DEPTH in intialization of class')
    ...
    def applyTransformKeypoint(self, image):
        ...
        if (self.data_type != "KEYPOINTS"):
            raise ValueError(
                "Please set dataset type to KEYPOINTS in intialization of class")
    ...
    def applyNoiseRoadWork(self):
        if (self.is_train):
            self.add_noise = self.transform_noise_roadwork(
                image=self.augmented_image)
            self.augmented_image = self.add_noise["image"]
