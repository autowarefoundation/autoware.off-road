...
from datamodule.utils import get_mean_std, build_segformer_train_augs
import albumentations as A
from albumentations.pytorch import ToTensorV2
import lightning as L

class GooseDataset(Dataset):
    def __init__(self, root_dir, split="train", crop_size=(512,512), scale_base=(2048,512), ratio_range=(0.5,2.0), mean=(123.675,116.28,103.53), std=(58.395,57.12,57.375), transform=None):
    ...
    def __getitem__(self, idx):
        ...
        image = out["image"]                                # torch.float32
        mask  = out["mask"].squeeze(0).long() 
        return image, mask


class GooseDataModule(L.LightningDataModule):
    def __init__(self, root, batch_size=4, num_workers=4, num_classes=12, img_size=(512,512), scale_base=(2048,512), ratio_range=(0.5,2.0), mean=(123.675,116.28,103.53), std=(58.395,57.12,57.375)):
        super().__init__()
        self.save_hyperparameters()
        self.train_ds = GooseDataset(root, "train", img_size, scale_base, ratio_range, mean, std)
        self.val_ds   = GooseDataset(root, "val",   img_size, scale_base, ratio_range, mean, std)
        self.test_ds  = GooseDataset(root, "test",  img_size, scale_base, ratio_range, mean, std)   
