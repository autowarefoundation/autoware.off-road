...
extern "C" {
    void* create_steering_controller(double wheelbase, double K_p, double K_i, double K_d) {
        return new SteeringController(wheelbase, K_p, K_i, K_d);
    }

    void destroy_steering_controller(void* handle) {
        if (handle) {
            delete static_cast<SteeringController*>(handle);
        }
    }

    double steering_compute(void* handle, double cte, double yaw_error, double forward_velocity, double curvature) {
        if (!handle) return 0.0;
        SteeringController* controller = static_cast<SteeringController*>(handle);
        return controller->computeSteering(cte, yaw_error, forward_velocity, curvature);
    }
}